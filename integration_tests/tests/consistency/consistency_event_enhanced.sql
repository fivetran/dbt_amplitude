...
-- this test ensures the amplitude__event_enhanced end model matches the prior version
with prod as (
    select {{ dbt_utils.star(from=ref('amplitude__event_enhanced'), except=exclude_cols) }}
