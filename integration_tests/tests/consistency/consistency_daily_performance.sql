...
-- this test ensures the amplitude__daily_performance end model matches the prior version
with prod as (
    select {{ dbt_utils.star(from=ref('amplitude__daily_performance'), except=exclude_cols) }}
